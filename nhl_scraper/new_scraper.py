    ...
    async def scrape_teams(self, source=None):
        ...
        if source == "active":
            try:
                ...
                return (pl.LazyFrame(teams).with_columns([
                    pl.col("commonName").struct.field("default").alias("commonName"),
                    pl.col("name").struct.field("default").alias("name"),
                    pl.col("placeNameWithPreposition").struct.field("default").alias("placeNameWithPreposition"),
                ])
            ...
            try:
                ...
                ]).drop(["firstSeason", "lastSeason", "id"]).collect())
            except Exception as e:
                print(f"Error scraping teams: {str(e)}")
                return pl.DataFrame()
...
class NHLScheduleScraper(NHLBaseScraper):
    ...
    async def scrape_schedule(self, progress=True):
        ...
        tasks = [self.fetch_schedule(team=team, season=season) for team in self.teams for season in self.seasons]
        results = []
        if progress:
        ...
        df = (pl.LazyFrame(schedule_data).with_columns([
            ...
            pl.col("awayTeam").struct.field("commonName").struct.field("default").alias("awayTeamCommonName"),
            pl.col("homeTeam").struct.field("commonName").struct.field("default").alias("homeTeamCommonName"),
            pl.col("awayTeam").struct.field("placeName").struct.field("default").alias("placeNameHome"),
            pl.col("homeTeam").struct.field("placeName").struct.field("default").alias("placeNameAway"),
            ...
            pl.col("winningGoalie").struct.field("firstInitial").struct.field("default").alias("winningGoalieFirstInitial"),
            pl.col("winningGoalie").struct.field("lastName").struct.field("default").alias("winningGoalieLastName"),
            pl.col("winningGoalScorer").struct.field("playerId").alias("winningGoalScorerId"),
            pl.col("winningGoalScorer").struct.field("firstInitial").struct.field("default").alias("winningGoalScorerFirstInitial"),
            pl.col("winningGoalScorer").struct.field("lastName").struct.field("default").alias("winningGoalScorerLastName"),
            pl.col("gameType").cast(pl.Utf8).replace(_SESSION_MAP).alias("_gameType"),
            pl.lit(datetime.now()).alias("meta_datetime")])
            .drop(["tvBroadcasts", "awayTeam", "homeTeam", "periodDescriptor","winningGoalie", "winningGoalScorer", "threeMinRecapFr", "condensedGameFr", "gameCenterLink", "threeMinRecap",	"condensedGame", "ticketsLink", "ticketsLinkFr"])
        )
        return df.collect()
    ...
    async def scrape_team_schedule(self, team=None, season=None):
        ...
        pl.lit(datetime.now()).alias("meta_datetime")).drop(["tvBroadcasts", "awayTeam", "homeTeam", "periodDescriptor","winningGoalie", "winningGoalScorer", "threeMinRecapFr", "condensedGameFr", "gameCenterLink", "threeMinRecap",	"condensedGame", "ticketsLink", "ticketsLinkFr"])
        except Exception as e:
...
class NHLTeamRosterScraper(NHLBaseScraper):
    ...
    async def scrape_team_rosters(self, progress=True):
        ...
        tasks = [self.fetch_team_roster(team, season) for team in self.teams for season in self.seasons]
        results = []
        ...
        return pl.DataFrame(roster_data).with_columns([
            pl.concat_str(pl.col("firstName").struct.field("default"), pl.col("lastName").struct.field("default"), separator=" ").alias("fullName"),
            ...
            pl.when(pl.col("positionCode").is_in(["G", "D"])).then(pl.col("positionCode")).otherwise(pl.lit("F")).alias("position"),
            pl.lit(datetime.now()).alias("meta_datetime")
        ])
...
class NHLTeamStatsScraper(NHLBaseScraper):
    ...
    async def scrape_team_stats(self, progress=True):
        ...
        tasks = [self.fetch_team_stats(team, season, session) for team in self.teams for season in self.seasons for session in self.sessions]
        results = []

        if progress:
            with tqdm(total=len(tasks), desc=f"Scraping NHL {'Goalie' if self.goalies else 'Skater'} Stats") as pbar:
                for task in asyncio.as_completed(tasks):
                    result = await task
                    results.append(result)
                    pbar.update(1)
        ...
        return (pl.LazyFrame(stats_data).with_columns([
            pl.concat_str(pl.col("firstName").struct.field("default"), pl.col("lastName").struct.field("default"), separator=" ").alias("fullName"),
            pl.col("firstName").struct.field("default").alias("firstName"),
            pl.col("lastName").struct.field("default").alias("lastName"),
            pl.when(pl.col("positionCode").is_in(["G", "D"])).then(pl.col("positionCode")).otherwise(pl.lit("F")).alias("position"),
...
class NHLDraftScraper(NHLBaseScraper):
    ...
    async def scrape_draft(self, progress=True):
        ...
        return pl.DataFrame(draft_data).with_columns([
            ...
            pl.col("teamPlaceNameWithPreposition").struct.field("default").alias("teamPlaceNameWithPreposition"),
...
class NHLDraftRankingsScraper(NHLBaseScraper):
    ...
    async def scrape_rankings(self, progress=True):
        ...
        tasks = [self.fetch_rankings(year, category) for year in self.years for category in self.categories]
        results = []
        ...
        return pl.DataFrame(rankings_data).with_columns([
            pl.concat_str(pl.col("firstName"), pl.col("lastName"), separator=" ").alias("fullName"),
            pl.lit(datetime.now()).alias("meta_datetime")
        ])
...
class NHLTOIScraper(NHLBaseScraper):
    ...
    def __init__(self, game_ids):
        self.game_ids = [str(game_id) for game_id in game_ids] if isinstance(game_ids, (list, tuple)) else [str(game_ids)]
    ...
    async def scrape_toi_single_game(self, game_id):
        ...
        final_dict['TOI_combined'] = pl.concat([df for df in [final_dict["awayTOI"], final_dict["homeTOI"]] if df is not None], how="diagonal").with_columns(pl.lit(final_dict['GameId']).alias('gameId'))
        final_dict['Summary_combined'] = pl.concat([df for df in [final_dict["awaySummary"], final_dict["homeSummary"]] if df is not None], how="diagonal").with_columns(pl.lit(final_dict['GameId']).alias('gameId'))
        return final_dict
    ...
    async def _fetch_and_parse_team_toi(self, url, team_l, final_dict, browser):
        ...
        try:
            ...
            n_trs = len(parser.css("body > div.pageBreakAfter > table > tbody > tr:nth-child(4) > td > table > tbody > tr"))
            players = [
                parser.css(f"body > div.pageBreakAfter > table > tbody > tr:nth-child(4) > td > table > tbody > tr:nth-child({i+1}) > td.playerHeading")[0].text()
                for i in range(n_trs)
                if parser.css(f"body > div.pageBreakAfter > table > tbody > tr:nth-child(4) > td > table > tbody > tr:nth-child({i+1}) > td.playerHeading")
            ]
            team = parser.css("body > div.pageBreakAfter > table > tbody > tr:nth-child(3) > td > table > tbody > tr > td")[0].text()
            ...
            columns_1 = ["Shift #", "Per", "Start of Shift - Elapsed / Game", "End of Shift - Elapsed / Game", "Duration", "Event"]
            columns_2 = ["Per", "SHF", "AVG", "TOI", "EV TOT", "PP TOT", "SH TOT"]

            toi_df1, toi_df2 = [], []
            for key, shifts in player_dict.items():
                df_1 = pl.DataFrame([i for i in shifts if len(i) == 6], schema=columns_1, orient="row").with_columns(pl.lit(key).alias("Player"))
                df_2 = pl.DataFrame([i for i in shifts if len(i) == 7], schema=columns_2, orient="row").with_columns(pl.lit(key).alias("Player"))
            ...
            if toi_df1 is not None:
                toi_df1 = toi_df1.with_columns([
                    pl.col("Player").str.split(" ").list.get(0).cast(pl.Int32, strict=False).alias("#"),
                    pl.col("Shift #").cast(pl.Int32, strict=False),
                    pl.col("Per").cast(pl.Int32, strict=False),
                    pl.col("Start of Shift - Elapsed / Game").str.split(" / ").alias("Start Elapsed"),
                    pl.col("End of Shift - Elapsed / Game").str.split(" / ").alias("End Elapsed"),
                ]).with_columns([
                    pl.col("Start Elapsed").list.get(0).alias("Start Elapsed Time - Legacy"),
                ...
                ]).drop(["Start Elapsed", "End Elapsed", "Start of Shift - Elapsed / Game", "End of Shift - Elapsed / Game"]).with_columns(
                    (pl.col("Duration").str.split(":").list.get(0).cast(pl.Int32, strict=False) * 60 +
                     pl.col("Duration").str.split(":").list.get(1).cast(pl.Int32, strict=False)).alias("Duration (Seconds)")
                ...
                for col in ['AVG', 'TOI', 'EV TOT', 'PP TOT', 'SH TOT']:
                    toi_df2 = toi_df2.with_columns(
                        (pl.col(col).str.split(":").list.get(0).cast(pl.Int32, strict=False) * 60 +
                         pl.col(col).str.split(":").list.get(1).cast(pl.Int32, strict=False)).alias(f"{col} (Seconds)")
                    )
                toi_df2 = toi_df2.with_columns(pl.lit(team).alias("Team"), pl.lit(team_l).alias("H/V"))
    ...
    async def scrape_toi_multiple_games(self, progress=True):
        ...
        valid_results = [r for r in results if isinstance(r, dict) and r.get('TOI_combined') is not None and not r['TOI_combined'].is_empty()]
        if valid_results:
            combined_toi = pl.concat([r['TOI_combined'] for r in valid_results], how="diagonal")
            combined_summary = pl.concat([r['Summary_combined'] for r in valid_results], how="diagonal")
            return {
                "TOI_combined": combined_toi,
                "Summary_combined": combined_summary,
                "game_details": {r['GameId']: {k: v for k, v in r.items() if k not in ['TOI_combined', 'Summary_combined']} for r in valid_results}
            }
        else:
            print("No valid TOI data retrieved from any games.")
            return {"TOI_combined": pl.DataFrame(), "Summary_combined": pl.DataFrame(), "game_details": {}}
...
class NHLPlayByPlayParser:
    ...
    def __init__(self):
        self.event_mapping = {
            "BLOCK": "blocked-shot", "DELPEN": "delayed-penalty", "FAC": "faceoff",
            "GIVE": "giveaway", "GOAL": "goal", "HIT": "hit", "MISS": "missed-shot",
            "PENL": "penalty", "SHOT": "shot-on-goal", "STOP": "stoppage", 
            "TAKE": "takeaway", "GEND": "game-end", "PEND": "period-end", 
            "PSTR": "period-start", "SOC": "shootout-completed"
        }
        self.event_columns = {
    ...
    def parse_html(self, html):
        ...
        for row in table:
            ...
            on_ice = [el.text() for el in row.css("td > table > tbody") if len(el.text(strip=True)) > 5]
    ...
    def process_rosters(self, api_data):
        ...
        abbrev_dict = {api_data['awayTeam']['id']: api_data['awayTeam']['abbrev'], api_data['homeTeam']['id']: api_data['homeTeam']['abbrev']}
        home_away = {api_data['awayTeam']['id']: 0, api_data['homeTeam']['id']: 1}

        return rosters.with_columns([
            pl.col("teamId").replace_strict(abbrev_dict, default="unknown").alias("Team").cast(pl.Utf8),
            pl.col("teamId").replace_strict(home_away, default="unknown").alias("Home/Away").cast(pl.Int32),
    ...
    def create_mappings(self, rosters):
        ...
        return {
            "home_names": dict(zip(home_roster['sweaterNumber'].cast(str), home_roster['fullName'])),
            "away_names": dict(zip(away_roster['sweaterNumber'].cast(str), away_roster['fullName'])),
    ...
    def map_players(self, df, mappings):
        ...
        def replace_list(series, mapping_dict):
            return [
                [mapping_dict.get(str(i), None) for i in row if i] if row is not None and len(row) > 0 else None
                for row in series
            ]
    ...
    def merge_with_api(self, df, api_data):
        ...
        df = df.with_columns([
            ...
            pl.col("Event").replace_strict(self.event_mapping, default="unknown").alias("event")
        ])
        api_df = api_df.with_columns([
            pl.arange(0, api_df.height).alias("index"),
            pl.col("typeDescKey").alias("event")
    ...
    def add_action_players(self, df, rosters):
        ...
        df = df.with_columns([pl.lit(None).alias(f'playerId_{i}') for i in range(1, 4)] + [pl.lit(None).alias(f'playerName_{i}') for i in range(1, 4)])

        for event, columns in self.event_columns.items():
            for i, col in enumerate(columns[:3], start=1):
                if col and col in df.columns:
                    df = df.with_columns(
                        pl.when(pl.col('typeDescKey') == event).then(pl.col(col)).otherwise(pl.col(f'playerId_{i}')).alias(f'playerId_{i}')
                    )

        columns_to_fill = ['details.awaySOG', 'details.homeSOG', 'details.awayScore', 'details.homeScore']
        ...
        df = df.with_columns([
            pl.col("details.goalieInNetId").cast(str, strict=False).replace(rosters_dict).alias("goalieInNetName")
        ]).with_columns([
            pl.col(f"playerId_{i}").cast(str, strict=False).replace(rosters_dict).alias(f"playerName_{i}")
            for i in range(1, 4)
        ])
        return df
    ...
    def add_on_columns(self, df, rosters, max_column_index):
        ...
        df = df.with_columns([
            pl.Series(f"home_skater_id{j+1}", [row[j] if row and j < len(row) else None for row in home_ids])
            for j in range(max_column_index)
        ] + [
            pl.Series(f"away_skater_id{j+1}", [row[j] if row and j < len(row) else None for row in away_ids])
            for j in range(max_column_index)
        ] + [
            pl.Series(f"home_skater_fullName{j+1}", [rosters_dict.get(row[j]) if row and j < len(row) and row[j] else None for row in home_ids])
            for j in range(max_column_index)
        ] + [
            pl.Series(f"away_skater_fullName{j+1}", [rosters_dict.get(row[j]) if row and j < len(row) and row[j] else None for row in away_ids])
            for j in range(max_column_index)
        ])
    ...
    def add_normalized_coords(self, df):
        ...
        df = df.with_columns([
            pl.col("details.xCoord").cast(pl.Float64),
            pl.col("details.yCoord").cast(pl.Float64)
        ])
        df = df.with_columns(
            pl.when(
                ((pl.col("details.eventOwnerTeamId") == pl.col("homeTeamId")) & (pl.col("homeTeamDefendingSide") == "right")) |
                ((pl.col("details.eventOwnerTeamId") == pl.col("awayTeamId")) & (pl.col("homeTeamDefendingSide") == "right"))
            ).then(0 - pl.col("details.xCoord")).otherwise(pl.col("details.xCoord")).alias("x")
        ).with_columns(
            pl.when(
                ((pl.col("details.eventOwnerTeamId") == pl.col("homeTeamId")) & (pl.col("homeTeamDefendingSide") == "right")) |
                ((pl.col("details.eventOwnerTeamId") == pl.col("awayTeamId")) & (pl.col("homeTeamDefendingSide") == "right"))
            ).then(0 - pl.col("details.yCoord")).otherwise(pl.col("details.yCoord")).alias("y")
        )
        return df

    def add_game_info(self, api_data, df):
        """Add game metadata to the DataFrame."""
        abbrev_id_dict = {api_data['awayTeam']['id']: api_data['awayTeam']['abbrev'], api_data['homeTeam']['id']: api_data['homeTeam']['abbrev']}
        return df.with_columns([
            ...
            pl.lit(datetime.strptime(api_data['startTimeUTC'], '%Y-%m-%dT%H:%M:%SZ')).alias("startTimeUTC"),
    ...
    def add_elapsed_time(self, df):
        ...
        df = df.with_columns(pl.col("timeInPeriod").map_elements(convert_str_to_sec, return_dtype=pl.Int64).alias("timeInPeriod_s"))
        df = df.with_columns(
            pl.when(pl.col("periodDescriptor.periodType") == "SO")
            .then(pl.lit(3900).alias("elapsedTime"))
            .otherwise((pl.col("timeInPeriod_s") + (pl.col("periodDescriptor.number") - 1) * 60 * 20).alias("elapsedTime"))
        )
        return df
    ...
    def add_strength(self, df):
        ...
        return df.with_columns([
            ...
            .otherwise(
                ...
                .otherwise(pl.concat_str([pl.col("awaySktrs"), pl.lit("v"), pl.col("homeSktrs")]))
            ).alias("strength")
        ])
    ...
    def rename_columns(self, df):
        ...
        rename_dict = {
            "typeDescKey": "api_event", "periodDescriptor_number": "periodNumber",
            "periodDescriptor_periodType": "periodType", "details_eventOwnerTeamId": "eventOwnerTeamId",
            "details_yCoord": "yCoord", "details_xCoord": "xCoord", "details_zoneCode": "zoneCode",
            "details_reason": "reason", "details_shotType": "shotType", "details_awaySOG": "awaySOG",
            "details_homeSOG": "homeSOG", "details_awayScore": "awayScore", "details_homeScore": "homeScore",
...
class NHLPBPProcessor:
    ...
    async def process_single_game(self, game_id):
        ...
        try:
            ...
            max_column_index = max(len(plyrs) for plyrs in home_on_ice + away_on_ice if plyrs is not None and len(plyrs) > 0) if any(home_on_ice + away_on_ice) else 6
            ...
            cols_to_drop = [
                "periodDescriptor.maxRegulationPeriods", "homeON", "awayON", "homeG", "awayG",
                "homeNames", "awayNames", "homeIds", "awayIds", "details.highlightClipSharingUrl",
                "details.highlightClipSharingUrlFr", "details.highlightClip", "details.highlightClipFr",
                "details.discreteClip", "details.discreteClipFr", "pptReplayUrl", "Time:Elapsed Game",
                "typeCode", "eventId", "sortOrder", "situationCode", "details.hittingPlayerId",
                "details.hitteePlayerId", "details.losingPlayerId", "details.winningPlayerId",
                "details.blockingPlayerId", "details.assist1PlayerId", "details.assist2PlayerId",
                "details.scoringPlayerId", "details.committedByPlayerId", "details.shootingPlayerId"
            ]
...
class Scraper:
    ...
    async def scrape_all(self, teams=None, seasons=None, sessions=None, game_ids=None, 
                        date_range=None, standings_date=None, years=None, categories=None, 
                        round_="all", goalies=False, progress=True):
        ...
        if teams and seasons and sessions:
            tasks.append(("stats", self.scrape_team_stats(teams, seasons, sessions, goalies, progress=False)))
        if game_ids:
