import pytest
import pandas as pd
from nhl_scraper.scraper.scrape import scrapeDraft
import requests
...
def test_scrapeDraft_basic(valid_year, valid_round):
    ...
    assert expected_columns.issubset(draft_df.columns), (
...
def test_scrapeDraft_specific_round(valid_year, specific_round):
    ...
    assert not draft_df.empty, "Draft DataFrame is empty"
    assert all(draft_df["round"] == specific_round), (
...
def test_scrapeDraft_data_types(valid_year, valid_round):
    ...
    assert pd.api.types.is_datetime64_any_dtype(draft_df["meta_datetime"]), "meta_datetime is not datetime"
...
def test_scrapeDraft_metadata(valid_year, valid_round):
    ...
    assert (pd.Timestamp.now() - draft_df["meta_datetime"].max()) < max_age, (
        "meta_datetime is not recent enough"
    )